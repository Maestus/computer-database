...
public class CompanyDAO extends ModelDAO {
	...
	public Optional<Long> create(Model model) throws Exception {
		...
		try (Connection connexion = Datasource.getConnexion();
				PreparedStatement preparedStatement = ModelDAO.initialisationRequetePreparee(connexion, SQL_INSERT,
						true, ((Company) model).getNom());) {
	...
	public Page<Company> findAll(int offset, int nbElem) throws DAOException {
		...
		if (nbElem == Page.NO_LIMIT) {
			try (Connection connexion = Datasource.getConnexion();
					PreparedStatement preparedStatement = ModelDAO.initialisationRequetePreparee(connexion,
							SQL_SELECT_ALL_NOLIMIT, false);) {
				resultSet = preparedStatement.executeQuery();
				if (nbElem != Page.NO_LIMIT) {
			...
			try (Connection connexion = Datasource.getConnexion();
					PreparedStatement preparedStatement = ModelDAO.initialisationRequetePreparee(connexion,
							SQL_SELECT_ALL, false, nbElem, offset);) {
				resultSet = preparedStatement.executeQuery();
				if (nbElem != Page.NO_LIMIT) {
					while (resultSet.next()) {
						p.addElem((Company) mapper.map(resultSet));
	...
	public void update(Model m) {
		...
		try (Connection connexion = Datasource.getConnexion();
				PreparedStatement preparedStatement = ModelDAO.initialisationRequetePreparee(connexion, SQL_UPDATE,
						false, m.getNom(), m.getId());) {
	...
	public void delete(long id) throws DAOException {
		...
		try (Connection connexion = Datasource.getConnexion();
				PreparedStatement preparedStatement = ModelDAO.initialisationRequetePreparee(connexion, SQL_DELETE,
						false, id);) {
			preparedStatement.executeUpdate();
		} catch (SQLException e) {
			Logger logger = LoggerFactory.getLogger("CompanyDAO.update.SQL");
	...
	public void remove(long id) {
		...
		try (Connection connexion = Datasource.getConnexion();
				PreparedStatement preparedStatement_0 = ModelDAO.initialisationRequetePreparee(connexion,
						SQL_DELETE_COMPUTER, false, id);
				PreparedStatement preparedStatement_1 = ModelDAO.initialisationRequetePreparee(connexion, SQL_DELETE,
						false, id);) {
